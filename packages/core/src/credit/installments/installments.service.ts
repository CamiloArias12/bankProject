...
export class InstallmentsService {
  ...
  async finOneByCreditAndNumberInstallment(
    idCredit: number,
    numberInstallment: number,
  ): Promise<Installment> {
    return await this.installmentRepository.findOne({
      where: { id_credit: idCredit, installmentNumber: numberInstallment },
    });
  }
  ...
  async updateState(
    ...
    await this.installmentRepository.update(
      { installmentNumber: installmentNumber, id_credit: credit },
      { state: state },
    );
  }
}
