...
async function getSavingesByClient(identification:number): Promise<Saving[]> {
 ...
 query ($identification:Float!){
  ...
  const { data } = await getClient().query({ query: SAVINGS,variables:{identification:identification}});
...
async function PageSaving() {
   ...
   const savings: Saving[] = data===Role.EMPLOYEE ? await getSavinges(): await getSavingesByClient(Number(identification));
