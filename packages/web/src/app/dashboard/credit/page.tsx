...
async function getCreditsClient(identification:number): Promise<Credit[]> {
  ...
  `;
  const { data } = await getClient().query({ query: CREDITS_CLIENT ,variables:{identification:identification}});
...
async function PageCredit() {
  ...
  const credits: Credit[] = data===Role.EMPLOYEE ? await getCredits(): await getCreditsClient(Number(identification));
  return (
    <>
