...
query ($identification:Float!){
  ...
  const { data } = await getClient().query({ query: CDTS_CLIENT,variables:{identification:identification}});
...
async function PageCdt() {
  ...
  const cdts: Cdt[] = data===Role.EMPLOYEE ? await getCdtes(): await getCdtesClient(Number(identification));
  return (
    <>
      <Cdts cdts={cdts} />
    </>
